...
| Focused anonymous-response suite | Pass | 59/59 tests, including nullable roots, object/array undefined-root, array void-root and nested value-position-undefined rejection, generic interface/type-alias rejection with monomorphic-alias positives, direct and array-wrapped anonymous root-union rejection, incomplete anonymous-array rejection, top-level array metadata preservation, manual union/nullable/array boundaries, default/off preservation, canonical aliased/qualified/nested/transformed containers, non-canonical lookalikes, and `PromiseLike` rejection |
| Core regression suite | Pass | 15 files, 134 tests passed after config, transform/non-JSON-boundary, scoped anonymous-array route and whole-shape gating, direct/mixed-root-union, generic-reference and value-position-undefined hardening, decorator/type-provenance, barrel-shadowing, nullable-root/schema, semantic-container, array-metadata, and component-completeness hardening |
| Fresh uncached workspace build | Pass | `pnpm.cmd exec turbo run build --force`: 6/6 tasks, 0 cached, six packages, 6.419s |
| Fresh uncached workspace test | Pass | `pnpm.cmd exec turbo run test --force`: 12/12 tasks, 0 cached, 22 files and 162 tests across six packages, 53.673s |
...
| Local branch audit | Pass | Worktree clean; branch diff check and public naming scan passed |
...
The system cannot infer runtime serialization/interceptor/filter effects, manually written responses, open records/index signatures, unknown or `any` members, value-position `undefined`, direct generic schema declarations, callable/constructable objects, classes/framework wrappers, direct or array-wrapped multi-shape anonymous root unions, complex nested unions, or incomplete nested shapes. Those cases deliberately remain unresolved and retain their override path. Default/off keep the pre-existing root-union `oneOf` behavior; safe mode alone applies the stricter contract.

## Codebase Metrics

| Metric | Value |
| --- | ---: |
| Final branch commits (including report sync) | 41 |
| Final tracked files changed | 11 |
| Final insertions | 3,635 |
| Final deletions | 38 |
| Current core suite | 15 files, 134 tests |
| Fresh workspace suite | 22 files, 162 tests across six packages |
...
| Consume `undefined` only at optional properties | Optionality is representable through the required-property set; `undefined` in an array item or other value position would otherwise be silently erased |
| Reject direct generic schema declarations | Bare component names cannot distinguish `Box<string>` from `Box<number>`; monomorphically named aliases remain safe because they provide stable component identities |
...
| A partial or metadata-losing schema is accepted | Medium | Enforce recursive type and emitted-schema completeness; consume `undefined` only at optional properties; reject direct generic component identities; focused regressions cover unsafe and undefined-like roots, value-position undefined, generic collisions, monomorphic aliases, nullable root emission, nested shapes, unsafe anonymous arrays, nullable schemas, and nested/top-level array item metadata |
